(ns helion-energy.main
  "Kotodama WASM entrypoint for the Helion Energy clean-room actor (L4) — Clojure port.
  ...
  facts (`helion_energy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "helion_energy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"helion_energy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "helion_energy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :helion_energy-compat/ready)
